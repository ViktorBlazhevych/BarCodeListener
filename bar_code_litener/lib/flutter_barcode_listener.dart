...
class BarcodeKeyboardListener extends StatefulWidget {
  ...
  final BarcodeScannedCallback _onDebugBarcodeScanner;
  ...
  BarcodeKeyboardListener(
      ...
      /// Callback to be called when barcode is debuged.
      required Function(String) onDebugBarcodeScanner,
      ...
      : _onBarcodeScanned = onBarcodeScanned,
        _onDebugBarcodeScanner = onDebugBarcodeScanner,
...
class _BarcodeKeyboardListenerState extends State<BarcodeKeyboardListener> {
  ...
  void onKeyEvent(String? char) {
    _onDebugBarcodeScanner
        .call("onKeyEvent -> $char (${utf8.encode(char ?? "").toList()})\n\n");

    final bytes = utf8.encode(char ?? "");
    _onDebugBarcodeScanner.call("onKeyEvent -> Bytes: $bytes\n");
    for (final byte in bytes) {
      _onDebugBarcodeScanner.call(
          "onKeyEvent -> Byte Value [$byte] Converted to hex 0x${byte.toRadixString(16)}\n\n");
    }
  ...
  void _keyBoardCallback(RawKeyEvent keyEvent) {
    _onDebugBarcodeScanner.call(
        "keyBoardCallback -> keyEvent: {$keyEvent} \nkeyData: ${keyEvent.data}\nkeyId: {$keyEvent.logicalKey.keyId}\n\n");
